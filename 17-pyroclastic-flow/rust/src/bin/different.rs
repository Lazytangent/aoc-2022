...
//             let repeats = (1_000_000_000_000 - idx) / (i - idx) - 1;
//             i += (i - idx) * repeats;
...
//     println!("Part one: {p1}");
//     println!("Part two: {}", total_height + get_height(&map))
// }
...
fn can_fit(map: &[[u8; 7]], rock: &[(usize, usize)], h: usize, w: usize) -> bool {
    rock.iter()
        .all(|(dh, dw)| w + dw < 7 && map[h + dh][w + dw] != b'#')
}
...
fn main() {
    let input = include_str!("../../../data/small.txt");
